...
int main(int argc, char *argv[]) {
  ...
  if (d.HasParseError()) {
    ...
    std::cout << " \nParsing Error :: " << d.GetParseError()
              << "  offset error : " << d.GetErrorOffset();
    return 1;
  }
    ...
    if (tagName == "map") {
      ...
      for (auto &innerObjs : m.value.GetObject()) {
        ...
        if (innerTagName == "hostVehicle") {
          std::cout << "\nobjectId :"
                    << innerObjs.value["objectId"].GetDouble();
          std::cout << "\nmodel:type :"
                    << innerObjs.value["model"]["type"].GetString();
          ...
          std::cout << "\nvelocity: "
                    << innerObjs.value["velocity"].GetDouble();
          std::cout << "\nlongitudinal_acceleration: "
                    << innerObjs.value["longitudinal_acceleration"].GetDouble();
          std::cout << "\nlateral_acceleration :"
                    << innerObjs.value["lateral_acceleration"].GetDouble();
        } else if (innerTagName == "vehicles") {
          for (int i = 0; i < innerObjs.value.Size(); i++) {
            std::cout << "\nobjectId :"
                      << innerObjs.value[i]["objectId"].GetDouble();
            std::cout << "\nmodel:type :"
                      << innerObjs.value[i]["model"]["type"].GetString();
            std::cout << "\nx :" << innerObjs.value[i]["x"].GetDouble();
            std::cout << "\ny :" << innerObjs.value[i]["y"].GetDouble();
            std::cout << "\nheading :"
                      << innerObjs.value[i]["heading"].GetDouble();
            std::cout << "\nwidth: " << innerObjs.value[i]["width"].GetDouble();
            std::cout << "\nlength: "
                      << innerObjs.value[i]["length"].GetDouble();
            std::cout << "\nvelocity: "
                      << innerObjs.value[i]["velocity"].GetDouble();
            std::cout
                << "\nlongitudinal_acceleration: "
                << innerObjs.value[i]["longitudinal_acceleration"].GetDouble();
            std::cout << "\nlateral_acceleration :"
                      << innerObjs.value[i]["lateral_acceleration"].GetDouble();
          }
        }
      }
