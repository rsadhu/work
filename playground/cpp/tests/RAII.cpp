...
public:
  ...
  // copy constructor
  Base(const Base &rhs){PRINTF(*this)}

  // move constructor
  Base(Base &&rhs){PRINTF(*this)}
  ...
  Base &operator=(Base &&rhs) { PRINTF(*this) return *this; }

  // virtual
  void func() { PRINTF(*this); }

  virtual ~Base() { PRINTF(*this) }
};
