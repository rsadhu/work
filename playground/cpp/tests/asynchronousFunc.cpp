...
template <typename T> class MyThread {
...
template <typename T> void MyThread<T>::start(T data, callbackFunction cb) {
  mData = std::move(data);
  mCb = cb;
  mThread = std::thread(&MyThread::run, this);
}

template <typename T> void MyThread<T>::wait() { mThread.join(); }

// thread function
template <typename T> void MyThread<T>::run() {
  mData++;
  std::this_thread::sleep_for(
      std::chrono::seconds(5)); // td::chrono::seconds(1)
  mCb(mData);
}
...
int main(void) {
  ...
  std::future<int> recd =
      std::async(std::launch::async, processDataAsync, std::ref(f));
  pr.set_value(10);
  auto data = recd.get();
  ...
  auto cb = [](int d) {
    std::cout << " data arrived is " << d << "Thread Id "
