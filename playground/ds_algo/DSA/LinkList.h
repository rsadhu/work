...
template <typename T> void LinkList<T>::insert(int index, T value) {
  if (index == 0) {
...
template <typename T> T LinkList<T>::valueIter(Node<T> *root, int n) {
  std::stack<T> st;
  while (root) {
    st.push(root->__data);
    root = root->__next;
...
template <typename T> void LinkList<T>::remove_value(T value) {
  if (!__root)
    return;
  Node<T> *prev, *cur = __root;
  prev = cur;
...
template <typename T> void LinkList<T>::push_front(T value) {
  Node<T> *tmp = new Node<T>(value);
  tmp->__next = __root;
  __root = tmp;
}
...
template <typename T> void LinkList<T>::push_back(T value) {
  Node<T> *it;
...
template <typename T> T LinkList<T>::value_at(int index) {
  int i = 0;
...
template <typename T> LinkList<T> &LinkList<T>::operator<(T d) {
  __size++;
  if (!__root) {
    __root = new Node<T>(d);
  } else {
