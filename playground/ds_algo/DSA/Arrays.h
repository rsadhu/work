...
template <typename T> Vector<T>::Vector(int capacity, int element) {
  if (capacity == 0)
    capacity = N;
  __capacity = capacity;
...
template <typename T> Vector<T>::Vector(std::initializer_list<T> &rhs) {
  Vector(rhs.size());
  for (auto it : rhs) {
    __data[__size++] = it;
  }
...
template <typename T> void Vector<T>::insert(int index, T item) {
