...
namespace {
template <typename T, typename U = int> U test1(T a, U b) { return a + b; }

template <class T, class U> int test2(T &a, U &b) { return a + b.length(); }

template <typename T> T adder(T v) { return v; }

template <typename T, typename... Args> T adder(T first, Args... vargs) {
  return first + adder(vargs...);
}

// template<>
void specFunc() { std::cout << "\n specFunc\n"; }

} // namespace

template <typename T> void dosomething(T a) {
...
template <typename T> void Utility<T>::runSomething(T a) {}

template <typename T> void Utility<T>::runInsideTemplate(T a) {}

template <typename T> T addNumbers(T a, T b);

template <> int addNumbers(int a, int b) { return a + b; }

template <> float addNumbers(float a, float b) { return a + b; }
...
int main(void) {
  ...
  std::cout << "5. variaditic templats .."
