...
template <typename T> T fun(T v) {
  std::cout << " implementation called\n";
  return v;
}

template <typename T, typename... Args> T fun(T first, Args... rest) {
  std::cout << " fun called\n";
...
template <typename... Args> std::vector<std::string> to_string(Args... rest) {
