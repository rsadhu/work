...
QmlNetworkClass::QmlNetworkClass(QObject *parent) : QObject(parent) {
  ...
  connect(m_networkManager.get(),
          ...
          [this]() { emit resultArrived(Result::PASSED); });
...
void QmlNetworkClass::connectToHost(const QString &h, int port) {
  (void)port;
  if (m_networkManager) {
    QUrl url(h);
...
void QmlNetworkClass::sendRequest(QByteArray &,
                                  std::function<void(QByteArray)> func) {
