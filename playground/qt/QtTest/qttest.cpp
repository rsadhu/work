...
QtTest::QtTest(QObject *parent) : QObject(parent) {
  ...
  connect(this, &QtTest::sendMessage, this, &QtTest::onMessageArrived,
          connectionType);

  //  connectionType = Qt::DirectConnection;

  connect(
      this, &QtTest::changeSomethingWithCopy, this,
      [](Data d) { qDebug() << " copy slot called==== >>1 "; }, connectionType);

  connect(
      this, &QtTest::changeSomethinWithCRefs, this,
      [](const Data &refDa) { qDebug() << " ref const  slot called ====> 2"; },
      connectionType);
