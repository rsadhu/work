...
int maxSum(std::vector<int>v)
{
	...
	int fSum = sum, bSum = sum, dSum = sum, Tot;
	for (int f = 0, b = v.size() - 1; f < v.size(); f++, b--)
	{
		fSum -= abs(v[f]);
		bSum -= abs(v[b]);
		dSum -= (abs(v[f]) + abs(v[b]));
...
bool composeKPalindromes(std::string s, int k) {
...
int main(int argc, char**argv) 
{
	...
	//s.display();
	//s.removeItem(1);
	//std::cout << "DISCO DISCO DISCO DISCO\n";
	//s.display();
	composeKPalindromes("abrarba", 2);
