...
template <typename T> unsigned int LinkList<T>::size() const { return m_size; }

template <typename T> bool LinkList<T>::empty() const { return m_size == 0; }

template <typename T> T LinkList<T>::value_at(unsigned int index) {
...
template <typename T> void LinkList<T>::push_front(const T &rhs) {
...
template <typename T> T LinkList<T>::pop_front() {
...
template <typename T> T LinkList<T>::pop_back() {
...
template <typename T> T LinkList<T>::front() {
  return m_root ? m_root->data : T();
}

template <typename T> T LinkList<T>::back() {
  Node<int> *tmp = nullptr;
  for (tmp = m_root; tmp->next; tmp = tmp->next)
    ;
...
template <typename T> void LinkList<T>::insert(int index, const T &rhs) {
  if (index == 1)
    push_front(rhs);
  else {
    auto cnt = 0;
    for (auto prev = m_root, cur = m_root->next; cur->next;
...
template <typename T> void LinkList<T>::erase(int index) {
  if (index == 1)
    pop_front();
  else {
    auto cnt = 0;
...
template <typename T> T LinkList<T>::value_n_from_end(int n) {
  auto index = m_size - n;
...
template <typename T> void LinkList<T>::remove_value(const T &rhs) {
  if (m_root) {
    if (rhs == m_root->data) {
      pop_front();
    } else {
...
template <typename T> void LinkList<T>::removeAllSentItems(const T &rhs) {
